...
It is easy to write code to verify (1) and (2). I have posted a script, lseries_exs.py, at my github repository [IntroToLfunctions](https://github.com/Joe-Ferrara/IntroToLfunctions), that shows the convergence of (1) and (2) when you sum different numbers of terms from the defining series for $L(\chi_{-4}, 1)$ and $L(\chi_{8}, 1)$. For instance, the first 5 decimals of the sum of the first 100,000 terms of $L(\chi_{-4}, 1)$ agrees with the first 5 decimals of $\frac{\pi}{4}$, and the first 9 decimals of the sum of the first 100,000 terms of $L(\chi_8,1)$ agrees with the first 9 decimals of $\frac{\log(1 + \sqrt{2})}{\sqrt{2}}$.
...
$$\zeta_K(s) = \sum_{\mathfrak a\subset\mathscr O_K}\frac{1}{N\mathfrak a^s} = \prod_{\mathfrak p\subset\mathscr O_K}\left(1 - \frac{1}{N\mathfrak p^s}\right)^{-1},$$
