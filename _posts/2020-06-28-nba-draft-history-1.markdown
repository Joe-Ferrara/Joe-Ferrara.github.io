...
For the rest of this post, I explain the aggregate statistic I created. For any player drafted in the top 60 picks between 1969 and 2013, the aggregate statistic gives a number between 0 and 10 that evaluates the entirety of the player's career in the NBA. This aggregate statistic is then used to do an analysis of the historically best teams at drafting NBA players and colleges at producing NBA players. This analysis will be the content of my following two posts for this project.

### The Aggregate Statistic

The goal of the aggregate stat is to give one number for each player that represents the career value of that player. For a given player, the statistics I have to work with to create this aggregate statistic are the stats in the draft history data. These stats fall into three categories: Longevity, Standard, and Advanced. The Longevity statistics are total years played, total games played, and total minutes played. The Standard statistics are total points, total rebounds, total assists, career field goal percentage, career three-points shooting percentage, career free throw shooting percentage, career minutes per game, career points per game, career rebounds per game, and career assists per game. The Advanced statistics are win shares (WS), win shares per 48 minutes (WS/48), box score plus minus (BPM), and value above replacement player (VORP).

To make the aggregate statistic, I focused on the Longevity and Advanced statistics. While the standard statistics are useful, they focus too much on offensive production, and the important aspects of them are accounted for in the advanced statistics. The advanced statistics are useful because they each measures something slightly different in an attempt to give a number that represents the value of a player over his career. The all time leaders for each advanced statistics are found on basketball-reference.com at the following links: [win shares](https://www.basketball-reference.com/leaders/ws_career.html), [win shares per 48 minutes](https://www.basketball-reference.com/leaders/ws_per_48_career.html), [box score plus minus](https://www.basketball-reference.com/leaders/bpm_career.html), and [value above replacement player](https://www.basketball-reference.com/leaders/vorp_career.html).

The philosophy of my approach to make the aggregate statistic is wisdom of the crowd. I want to combine in a sensible way all of the Advanced and Longevity statistics to produce one number between 0 and 10 to evaluate a players career. I did this in three steps. I created an **Agg1** statistic and an **Agg2** statistic which are each slightly different aggregates of the Advanced and Longevity player statistics. The final aggregate statistic, called **Agg**, is then the average of **Agg1** and **Agg2**.

The players used to to create **Agg** are the players drafted during the years 1969 - 2013 drafted in the top 60 picks that appeared in 100 or more NBA games. The year 1969 was chosen because it is after territorial picks were eliminated from the NBA draft, and the year Kareem Abdul-Jabbar was drafted. I stopped at the year 2013 because for more recent drafts it is difficult to evaluate a players career because it is still going on. By including drafts up to 2013 we still get a number for active players like Stephen Curry and Giannis Antetokounmpo. Only the top 60 picks were used because in the modern NBA draft there are only 60 picks. The 100 games restrictions makes it so that only players that appeared in a over a full season of NBA games are considered. The advanced statistics can be thrown off for players appearing in very few games, and a large portion of NBA draft picks do not appear in many games, so I did not want these players throwing off the statistics. The players drafted in the top 60 that appear in less than 100 games are then put back into the data and assigned a low **Agg** score when doing the draft analysis in my following posts.

#### Aggregate Stat 1

Using the wisdom of the crowd philosophy to get the good qualities from the statistics without relying too much on any individual one, **Agg1** will consist of an average of the Longevity and Advanced statistics. Since each of the statistics has a different range of values, taking a direct average is not the right thing to do because the statistics with a higher range of values will have a greater impact on the final number than the statistics with a smaller range of values. Therefore, all the statistics were min/max normalized over all the players, so that each them falls in the range 0 to 1.

After normalizing, for the Advanced statistics a an average was taken.

To incorporate the Longevity statistics I did something slightly different. The reason for including the Longevity statistics is because they are an important aspect of the value of draft pick that is not emphasized enough in the Advanced statistics. In addition to the Longevity statistics mentioned above, I also added a statistic to account for games missed because of injury. This statistic is average games per year. Average games per year is the players total games played divided by 82 times total years played. There are now four Longevity statistics: years played (Y), games played (G), minutes played (M), and average games per year (G/Y). To incorporate these Longevity statistics, into the aggregate statistic without them having too large an impact, I took their average, and then made them 10% of the aggregate statistic.

Finally, for aesthetic reasoned, I made the aggregate statistic fall between 0 and 10.

Given an Advanced or Longevity statistic, WS, WS/48, BPM, VORP, Y, G, M, G/Y, the normalized statistic be denoted with a subscript n (so WS<sub>n</sub>, WS/48<sub>n</sub>, BPM<sub>n</sub>, etc). Then **Agg1** is determined by the following formula:

**Agg1** = (1/4)\*(Y<sub>n</sub> + G<sub>n</sub> + M<sub>n</sub> + G/Y<sub>n</sub>) + (9/4)\*(WS<sub>n</sub> + WS/48<sub>n</sub> + BPM<sub>n</sub> + VORP<sub>n</sub>).

To evaluate **Agg1**, I used it to produce a list of the top 20 players drafted between 1969 and 2013, which is displayed below.
...
This list is pretty good. In any list of the top players since 1969, the top three should be LeBron James, Michael Jordan, and Kareem Abdul-Jabbar in some order, which this list gets correct. In this list, there are no huge outliers; all the players had great NBA careers, and are or will be in the NBA Hall of Fame. The main drawback I see to this list is that it overvalues the modern player. In particular, Magic Johnson and Larry Bird should be higher, and there should be more players from the 70s and 80s.
...
This list again is pretty solid, but also has some drawbacks. Magic Johnson now appears in the top 5, and Larry Bird has moved up 2 spots. We also see that Julius Erving is ranked number 6, where as he did not appear in the previous list. Unfortunately, Michael Jordan has moved down to number 10, which is way too low, and Hakeem Olajuwon has disappeared from the list. Other pros of this list are that John Stockton and Karl Malone have moved down and Shaquille O'Neal has moved up.
...
While this list has its pros and cons as well, I think it is better that the ones produced by **Agg1** and **Agg2**. It has the correct top 3 players, and Tim Duncan is arguably the fourth best player since 1969. Some players are perhaps too high because of their lack of playoff success that **Agg** cannot account for. Overall, I like this group of 20. For the utility of **Agg**, it is more important that the group of 20 is solid without outliers than that the exact order is perfect since this statistic will be used to evaluate all players.
...
ESPN's top 20 and **Agg**'s top 20 have 16 players in common. **Agg1** and **Agg2**'s top 20 both have 15 players in common with ESPN's list.
